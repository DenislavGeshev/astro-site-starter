 ...
 * Sanity or Payload (see Part 5 and Part 9 of the build guide).
 *
 ...
 *   credentials in .env. Keep the function signatures the same."
 */
...
export async function getAllPosts(limit?: number): Promise<Post[]> {
  ...
  //     `${import.meta.env.PAYLOAD_API_URL}/api/posts?limit=${limit ?? 100}&sort=-publishedAt`
  //   );
