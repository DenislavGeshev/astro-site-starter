  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Trigger
        ...
        className="flex items-center justify-center w-10 h-10 rounded-md text-brand-dark hover:bg-brand-border/50 transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-primary"
      >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 right-0 z-50 w-full max-w-sm bg-brand-light p-6 shadow-xl data-[state=open]:animate-in data-[state=open]:slide-in-from-right">
            ...
            <ul className="list-none m-0 p-0 space-y-1">
              {items.map((item) => (
                <li key={item.href}>
                  <a
                    href={item.href}
                    className="block px-4 py-3 rounded-md text-lg font-medium text-brand-dark hover:bg-brand-border/30 transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-primary"
                  >
                    {item.label}
                  </a>
